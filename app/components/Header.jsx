  ...
  return (
    <div className='w-11/12 max-w-3xl text-center mx-auto h-screen flex flex-col justify-center items-center gap-4'>
        ...
        transition={{duration:0.8, type:"spring", stiffness:100}}>
            <Image src={user_img} alt="profile image" className='rounded-full w-32 mt-20' />
        </motion.div>
        <motion.h3
        initial={{y:-20, opacity:0}}
        whileInView={{y:0, opacity:1}}
        transition={{duration:0.6, delay:0.3}}
         className='flex items-end gap-2 text-xl md:text-2xl mb-3 font-bold'>Hi I'm Arghyadeep Dutta <Image src={assets.hand_icon} alt="hand icon" className='w-6' /></motion.h3>
        <motion.h1
        initial={{y:-30, opacity:0}}
        whileInView={{y:0, opacity:1}}
        transition={{duration:0.8, delay:0.5}}
        className='text-3xl sm:text-6xl lg:text-[66px] font-bold'>React Js and Next Js Developer
        </motion.h1>
        <motion.p
        initial={{ opacity:0}}
        whileInView={{ opacity:1}}
        transition={{duration:0.6, delay:0.7}}
        className='max-w-2xl mx-auto'>I am a skilled React.js and Next.js developer with a passion for creating user-friendly and visually stunning web applications.</motion.p>
